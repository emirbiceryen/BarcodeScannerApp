...
interface Record {
  ...
  productName: string;
  quantity: number;
  batch_no: string;  // batch_no ekledik
  kullanici: string;
}

const Cikis = ({ onBack }: { onBack: () => void }) => {
  const [data, setData] = useState<Record[]>([]); // Veritabanından alınan veriler

  // Veritabanındaki cikislar_takip tablosundan verileri çekme
  const fetchCikisData = async () => {
    try {
      const response = await fetch('https://67be-85-159-70-106.ngrok-free.app/api/Cikis'); // API uç noktasını güncelledik
  ...
  useEffect(() => {
    fetchCikisData(); // Sayfa açıldığında verileri çekmek için
  }, []);
  ...
  const renderItem = ({ item }: { item: Record }) => (
    <View style={styles.tableRow}>
      ...
      <Text style={styles.tableCell}>{item.batch_no}</Text> {/* batch_no ekledik */}
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Çıkış Takip Verileri</Text>
